...
//	FeliCa 操作 ( communicateThruEX を使って FeliCa カードを操作する。 )
	var felica = async () => {
		...
		if (resdata.status === true) {
            ...
            // IDmを16進数の文字列に変換
            const idmHex = arrayToHex(resdata.IDm);
    
            // テキストボックスに値を出力
            const idmTextbox = document.getElementById('idmOutput');
            if (idmTextbox) {
                idmTextbox.value = idmHex; // テキストボックスにIDmを設定
            } else {
                console.error('IDmの出力先テキストボックスが見つかりません');
            }
    
            // メッセージ表示
            messageTitle.innerHTML = `カード検出成功: ${idmHex}`;
        } else {
            // エラーメッセージ表示
